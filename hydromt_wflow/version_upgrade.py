...
def _convert_to_wflow_v1(
    ...
    for key, variables in states_vars.items():
        name = get_config( f"state.{variables['wflow_v0']}", config=config,fallback=None)
    ...
    for key, variables in wflow_vars.items():
        name = get_config( f"input.{variables['wflow_v0']}", config=config, fallback=None)
    ...
    if get_config( "netcdf", config=config, fallback=None) is not None:
        ...
        config_out["output"]["netcdf_scalar"] = {
            "path": get_config( "netcdf.path", config=config, fallback="output_scalar.nc"),
        }
