...
Set LAI to False or run setup_laimaps first"
        ...
        if (self._MAPS["thetaR"] not in self.staticmaps.data) or (
            self._MAPS["thetaS"] not in self.staticmaps.data
        ):
            logger.error(
...
{start} and endtime to {end} in the toml."
    ...
    def clip_grid(self, region, buffer=0, align=None, crs=4326, inverse_clip=False):
        ...
        if self._MAPS["resareas"] in self.staticmaps.data:
            reservoir = self.staticmaps.data[self._MAPS["resareas"]]
        ...
        remove_lake = False
        if self._MAPS["lakeareas"] in self.staticmaps.data:
            lake = self.staticmaps.data[self._MAPS["lakeareas"]]
    ...
    def clip_states(self):
        ...
        if len(self.states) > 0:
            ...
            remove_maps = []
            if self._MAPS["resareas"] not in self.staticmaps.data:
                if "volume_reservoir" in ds_states:
                    remove_maps.extend(["volume_reservoir"])
            if self._MAPS["lakeareas"] not in self.staticmaps.data:
