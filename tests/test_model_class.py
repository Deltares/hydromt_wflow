...
def test_setup_reservoirs(source, tmpdir):
    ...
    required = ['ResDemand', 'ResMaxRelease', 'ResMaxVolume', 'ResSimpleArea', 'ResTargetFullFrac', 'ResTargetMinFrac']
    assert all(x==True for x in [k in mod1.staticmaps.keys() for k in required]), '1 or more reservoir map missing'
    ...
    for i in required:
        assert (np.count_nonzero(~np.isnan(staticmaps[i].sel(lat=stacked.lat.values, lon=stacked.lon.values))) == number_of_reservoirs), f'Number of non-null values in {i} not equal to number of reservoirs in model area'
