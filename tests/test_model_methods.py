...
def test_setup_reservoirs(source, tmpdir):
    ...
    assert all(
        x == True for x in [k in mod1.grid.keys() for k in required]
    ...
    grid = mod1.grid.where(mod1.grid.wflow_reservoirlocs != -999)
    stacked = grid.wflow_reservoirlocs.stack(x=["lat", "lon"])
    ...
    for i in required:
        assert (
            np.count_nonzero(
                ~np.isnan(grid[i].sel(lat=stacked.lat.values, lon=stacked.lon.values))
            )
