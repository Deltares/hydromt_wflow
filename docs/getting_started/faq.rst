...
basin/subbasins you want to derive, that was prepared using the same DEM source that you intend to use in the :py:func:`~WflowBaseModel.setup_basemaps`
method.
...
Yes, that is one of the reason why the two were separated into two separate methods: :py:func:`~WflowBaseModel.setup_precip_forcing` and
:py:func:`~WflowBaseModel.setup_temp_pet_forcing`, you can just use a different data source for *precip_fn* and *temp_pet_fn* and HydroMT
...
We do support conversion of Wflow v0.x Julia models to Wflow v1.x Julia models using the :py:func:`~WflowBaseModel.upgrade_to_v1_wflow` method.
